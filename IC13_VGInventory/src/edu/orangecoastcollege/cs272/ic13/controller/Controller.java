...
public class Controller {
	...
	private ObservableList<VideoGame> mFilteredVideoGamesList;
	...
	public static Controller getInstance() {
		if (theOne == null) {
			...
			theOne.mFilteredVideoGamesList = FXCollections.observableArrayList();
	...
	public ObservableList<VideoGame> getGamesForCurrentUser()
	{
		...
		try {
			ArrayList<ArrayList<String>> userGames = theOne.mUserGamesDB.getAllRecords();
			for (int i = 0; i < theOne.mUserGamesDB.getRecordCount(); ++i)
			{
				userGamesList.add(mAllGamesList.get(Integer.parseInt(userGames.get(i).get(1)) - 1));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public boolean addGameToUsersInventory(int selectedGame)  {
		
		String[] values = new String[2];
		values[0] = String.valueOf(theOne.mCurrentUser.getId());
		values[1] = String.valueOf(selectedGame);
	...
	{
		try {
			theOne.mUserGamesDB.deleteAllRecords();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
